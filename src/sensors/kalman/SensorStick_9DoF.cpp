...
void SensorStick_9DoF::sensorInit(void) {
  ...
  float accZero[3] = { 0 }; // accX, accY, accZ
  float gyrZero[4] = { 0 }; // gyroX, gyroY, gyroZ,gyroTemp
  float magZero[3] = { 0 }; // magX, magY, magZ
...
float SensorStick_9DoF::getZero(char sensor, char axis) {
  int i = 0;
  switch (axis) {
    case 'x': i = 0; break;
    case 'y': i = 1; break;
...
int SensorStick_9DoF::setZero(char sensor, char axis, float zero[]) {
  int i = 0;
  switch (axis) {
    case 'x': i = 0; break;
    case 'y': i = 1; break;
...
int SensorStick_9DoF::setZero(float acc[], float gyro[], float mag[]) {
  for (int i = 0; i < 3; i++) {
    _accZero[i] = acc[i];
    _gyroZero[i] = gyro[i];
    _magZero[i] = mag[i];
...
float SensorStick_9DoF::get(char sensor, char axis) {
  int i = 0;
  switch (axis) {
    case 'x': i = 0; break;
    case 'y': i = 1; break;
  ...
  switch (sensor) {
    case ACC: return (float)_acc[i] / accSensityivity[i]; break;
    case GYR: return (float)_gyro[i] / gyroSensityivity[i]; break;
    case MAG: return (float)_mag[i] / magSensityivity[i];; break;
  }
}
